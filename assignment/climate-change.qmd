...
**Answer 1:**
CO2 concentration has exponentially increased. From 1958 to current day, concentrations have increased annually.
**Question 2:**
...
**Answer 2:**
Co2 reaches a maximum c02 level in may and minimum in September. The Northern hemisphere seasonal patterns explains the pattern. During winter months vegetation is not actively absorbing c02 and vice versa during the summer. The northers hemisphere is what drives this due to more land mass and vegetation.
...
monthly <- co2 |> 
  group_by(month) |> 
  summarize(mean_av = mean(monthly_average)) |> 
  ggplot(aes(month, mean_av))+
  geom_line() +
  theme_classic()+
...
each column is numeric and is measured in Celsius
The difference between global temp & global temp anomaly is that the anomaly data is calculated by taking the difference between the average/baseline temperature and the recorded temp.
...
**Answer 4:**
```{r}
Global.temp <-  read_table("https://data.giss.nasa.gov/gistemp/graphs/graph_data/Global_Mean_Estimates_based_on_Land_and_Ocean_Data/graph.txt", 
                  comment="#",
                  col_names = c("year", "temp", "lowess"),
                  na = c("-1", "-99.99"), skip =5)
...
Average global temp started gradually increasing around 1920. The gradual increase can be see until around 1970, where average temp began to increase exponential till present day.
**Answer 5:**
```{r}
Global.temp |> ggplot(aes(year, temp)) + 
  geom_line() +
  labs(x = 'Year', y = 'Temp (C*)', title = 'Global mean temperature')+
   theme_minimal()
```
...
**Answer 6:**
I used the prompt 'how to calculate rolling averages in R' in ChatGPT. I used the answer that used package 'zoo' because I've never heard about it and it looked pretty simple. I just had to modify the data frame name/variables and width size for each average.
...
Annual average is the average using calculation from a single year and a 5-year average would be calculated finding the sum of 5 separate years of averages and dividing it by 5.
**Answer 7:**
```{r}
library(zoo)
Five_Year_Avg <- rollapply(Global.temp$temp, width = 5, FUN = mean, align = 'right', fill = NA)
Ten_Year_Avg <- rollapply(Global.temp$temp, width = 10, FUN = mean, align = 'right', fill = NA)
Twenty_Year_Avg <- rollapply(Global.temp$temp, width = 20, FUN = mean, align = 'right', fill = NA)

running_averages <- data.frame(Five_Year_Avg = Five_Year_Avg,
                               Ten_Year_Avg =Ten_Year_Avg,
                               Twenty_Year_Avg = Twenty_Year_Avg,
                               year = Global.temp$year) 
  
running_averages_long <- running_averages |> 
  pivot_longer(cols = c(Five_Year_Avg, Ten_Year_Avg, Twenty_Year_Avg),
               names_to = "Rolling_Avg_Type",
               values_to = "temp")
...
All three averages hold strong similarities in their patterns. However, some small differences can be observed. The 5 year average line shows greater variability, with more pronounced peaks and dips, indicating larger fluctuations in the data. Data variability seems to decreases through the 10 and 20 year averages, 10 year avr has less pronounced peaks and the 20 year average is almost completely smooth.
**Answer 8:**
```{r}
ggplot(running_averages_long, aes(x = year, y = temp, color = Rolling_Avg_Type)) +
  geom_line() + 
  labs(x = "Year", y = "Temperature (°C)", title = "Rolling Averages of global temperature") +
  theme_minimal()  
...
The only possible pause my graph shows is around 1970-1980 where there inst much change in temperature. I feel like the 20 year avr depicts the pause better than the others because temp is clearly depicted increasing right after 1920 until before 1960, which is where it  decreases and has very little change till 1980, and begins to increase exponentially till current day. 
...
**Answer 10:**
The columnns are Depth(m), Age of the ice (year), Mean age of the air(year), and C02 concentrations(ppmv)

Recent atmospheric CO2 levels are drastically higher than historical levels. The graph depicts this showing the maximum CO2 concentration in the historic data being around 300, while recent years have seen CO2 concentration reaching 425 ppvm. These recent levels have created a rapid exponential increase in the graph.
```{r}
vostock <- read_table("https://data.ess-dive.lbl.gov/catalog/d1/mn/v2/object/ess-dive-457358fdc81d3a5-20180726T203952542",col_names = c("Depth (m)", "year", "Mean age of air (yr BP)", 'CO2'), skip =20)
                 
vostock$year <- 2003 - vostock$year                    
                       
vostock |> ggplot(aes(x = year, y = `CO2 concentration (ppvm`)) + geom_line()   
...
combined <- bind_rows(vostock, co2)

ggplot(combined, aes(year, CO2)) + 
  geom_line() + 
  theme_minimal() +
  labs(x = 'Year', y = 'CO2 Concentration (ppvm)', title = 'Combined Polar Ice and Current C02 Data' )
```
